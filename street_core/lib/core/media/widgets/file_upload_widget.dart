// lib/core/media/widgets/file_upload_widget.dart
  ...
  Future<List<File>> _validateFiles(List<File> files) async {
    ...
    for (final file in files) {
      ...
      if (valid.length >= widget.maxFiles) {
        ...
        _showError(
          context.tr(
            'media.error.max_files_exceeded',
            args: {'max': widget.maxFiles.toString()},
          ),
        );
        break;
      }
      ...
      try {
        ...
        _showError(context.tr(LocaleKeys.mediaErrorReadingFile, args: {'name': fileName}));
        continue;
      }
  ...
  Widget _buildLoadingState() {
    return Column(
      ...
      children: [
        ...
        MyText(LocaleKeys.mediaInfoSelectingFiles),
      ],
    );
  }
  ...
  Widget _buildEmptyState() {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          '${LocaleKeys.mediaMax} ${widget.maxFiles} ${context.tr(widget.maxFiles > 1 ? 'files' : 'file')} • ${_formatBytes(widget.maxSizeBytes)}',
          style: TextStyle(
            fontSize: 11,
            color: theme.colorScheme.onSurfaceVariant,
          ),
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            Text(//TODO lOCALEkEYS ARRREGLAR PLURAL Y SIN GULAR
              '$_selectedFilesCount ${context.tr(_selectedFilesCount > 1 ? LocaleKeys.mediaHintFilesSelected : LocaleKeys.mediaHintFileSelected)}',
            ...
            if (_selectedFilesCount < widget.maxFiles)
              TextButton.icon(
                ...
                label: MyText(LocaleKeys.mediaHintAddMore),
              ),
          ],
        ),
        SizedBox(height: AppSpacing.md),
